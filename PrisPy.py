...
class Browser(object):
	def __init__(self, shortcut):
		...
		try:
			# Trying to load useragent from config file
			config.userAgent
		except NameError:
			# Do nothing if no useragent set in config file
			pass
		else:
			# Load it from webkit instead
...
class NewShortcut(object):
	def __init__(self):
		# Define some vars
		self.wantStatusbar = "0" # Needs to be a string value, as only strings can be written to config file
...
class Startup(object):
	...
	def delete_shortcut(self, widget, data=None):
		...
		model, iter = selection.get_selected()
		self.shortcut = model.get_value(iter, 0)
		# See if config files are there, and delete them
		try:
...
if __name__ == "__main__":
	# See if a certain page is started
	try: sys.argv[1]
	except IndexError:
		# No page; therefore start main window
		startup = Startup()
		startup.main()
	else:
		# Start browser with page given
		shortcut = sys.argv[1]
